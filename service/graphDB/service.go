...
var GraphDBURL string
var Repository string
var BaseQueryURL string
var VisualGraphURL string
var NodeDetailsURL string
var NodePropertiesURL string

// InitRequestURL init all request url.
func InitRequestURL() {
	GraphDBURL = viper.GetString("graphdb.addr")
	Repository = viper.GetString("graphdb.repository")
	BaseQueryURL = "http://" + GraphDBURL + "/repositories/" + Repository
	//var VisualGraphURL = "http://" + GraphDBURL + "/rest/explore-graph/links"
	VisualGraphURL = "http://" + GraphDBURL + "/rest/explore-graph/links"
	// http://localhost:7200/rest/explore-graph/node
	NodeDetailsURL = "http://" + GraphDBURL + "/rest/explore-graph/node"
	// http://localhost:7200/rest/explore-graph/node
	NodePropertiesURL = "http://" + GraphDBURL + "/rest/explore-graph/properties"

}

// QueryInfo query raw query from graphdb.
...
// GetVisualGraphInfo get info from graphdb visual graph.
func GetVisualGraphInfo(query string) ([]byte, error) {
	method := "GET"
	data := url.Values{}
	data.Set("config", "default")
	data.Set("includeInferred", "false")
	data.Set("iri", query)
	data.Set("preferredPredicatesOnly", "false")
	data.Set("preferredTypesOnly", "false")
	data.Set("sameAsState", "false")
	client := &http.Client{}

	req, err := http.NewRequest(method, VisualGraphURL, nil)
	if err != nil {
		return []byte{}, err
	}
...
func GetNodeDetails(node string) ([]byte, error) {
	...
	req, err := http.NewRequest(method, NodeDetailsURL, nil)
	if err != nil {
		return []byte{}, err
	}
	req.URL.RawQuery = data.Encode()
...
// GetNodeProperties get node properties.
func GetNodeProperties(node string) ([]byte, error) {
	method := "GET"
	data := url.Values{}
	data.Set("config", "default")
	data.Set("includeInferred", "false")
	...
	req, err := http.NewRequest(method, NodePropertiesURL, nil)
	if err != nil {
		return []byte{}, err
	}
	req.URL.RawQuery = data.Encode()
